...
impl BundleVariable {
  ...
  pub fn extend_used_name(&mut self, used_name: Vec<String>) {
    for name in used_name {
      self.uniq_name_mut().insert(&name);
    }
  }
  ...
  // A(entry, export * from B) -> B(export * from C) -> C
  pub fn try_find_export_all(
    &self,
    issuer: &ModuleId,
    source: &ModuleId,
    module_graph: &ModuleGraph,
    module_analyzers: &HashMap<ModuleId, ModuleAnalyzer>,
  ) -> Option<ZipExportAllChain> {
    struct FindOption {
      issuer: ModuleId,
      source: ModuleId,
    }
    ...
    let mut result = ZipExportAllChain {
      chains: vec![],
      is_use_declare_replace_export: false,
    };
    ...
    {
      let statements = module_analyzers
        .get(&dep_id)
        .map(|item| item.exports_stmts())
        .unwrap_or_else(|| vec![]);
      ...
      result.chains.push(dep_id.clone());

      result.is_use_declare_replace_export = sources.len() != 1;
...
pub struct ZipExportAllChain {
  pub chains: Vec<ModuleId>,
  pub is_use_declare_replace_export: bool,
}
