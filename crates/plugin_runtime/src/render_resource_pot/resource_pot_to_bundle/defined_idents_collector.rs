...
impl<'a> RenameIdent<'a> {
  ...
  fn in_key_map_nested(&mut self, f: impl FnOnce(&mut Self)) {
    self.in_key_value_mode = true;
    f(self);
    self.in_key_value_mode = false;
  }
}
...
impl<'a> VisitMut for RenameIdent<'a> {
  ...
  // fn visit_mut_prop_or_spread(&mut self, n: &mut farmfe_core::swc_ecma_ast::PropOrSpread) {
  //   match n {
  //     farmfe_core::swc_ecma_ast::PropOrSpread::Spread(m) => m.visit_mut_children_with(self),
  //     farmfe_core::swc_ecma_ast::PropOrSpread::Prop(m) => {
  //       match m {
  //           farmfe_core::swc_ecma_ast::Prop::Shorthand(_) => todo!(),
  //           _ => {
